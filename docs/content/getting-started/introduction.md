...
booking is a single binary. It speaks to booking over plain HTTPS,
shapes the responses into clean records, and gets out of your way. There is
nothing to sign up for and nothing to run alongside it.

## How it is built

- A **library package** (`booking`) holds the HTTP client and the typed
...
- A thin **`cmd/booking`** hands the assembled app to `kit.Run`, which
...
```bash
booking page <path>                  # the command
booking serve --addr :7777           # GET /v1/page/<path>
booking mcp                          # the page tool, over stdio
ant get booking://page/<path>        # the URI dereference (via a host)
```
...
booking is a read-only client over data booking already serves
